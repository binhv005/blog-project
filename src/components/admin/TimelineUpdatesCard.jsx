  ...
  return (
    <section className="rounded-2xl bg-[#1e1a26]/90 border border-[#2c2835]/80 p-6 flex flex-col justify-between shadow-[0_8px_32px_rgba(0,0,0,0.25)]">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-display font-bold text-lg text-on-surface">Cập nhật mới (MongoDB)</h2>
        <span className="px-2.5 py-0.5 rounded-full bg-[#ff5167]/15 text-[#ffb3b5] font-mono text-[11px] font-bold">
        ...
        {recentUpdates.map((item) => (
          ...
          >
            ...
            <div className="w-20 text-right flex flex-col flex-shrink-0 pt-0.5">
              <span className="font-mono text-xs font-semibold text-on-surface-variant truncate">{item.day}</span>
              <span className="font-mono text-[10px] text-on-surface-variant/60">{item.time}</span>
            </div>
            ...
            <div className="pt-1.5 flex flex-col items-center flex-shrink-0">
              ...
              {item.hasLine && <div className="w-0.5 h-10 bg-[#2c2835] mt-1" />}
            </div>

            {/* Content */}
            <div className="flex-1 min-w-0 pt-0.5">
              <span className={`font-display font-semibold text-sm transition-colors line-clamp-2 break-words block leading-snug group-hover:underline ${item.titleColor}`}>
                {item.title}
              </span>
              <span className="text-xs text-on-surface-variant block mt-0.5 break-words font-mono">
                {item.desc}
              </span>
            </div>
          </div>
