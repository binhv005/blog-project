    ...
    <section className="rounded-2xl bg-[#1e1a26]/90 border border-[#2c2835]/80 p-6 flex flex-col justify-between shadow-[0_8px_32px_rgba(0,0,0,0.25)]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h2 className="font-display font-bold text-lg text-on-surface">Lượt xem</h2>
          <span className="text-xs font-mono text-[#4cd7f6] bg-[#4cd7f6]/10 px-2 py-0.5 rounded-md font-semibold">
            {totalViews.toLocaleString()} views (MongoDB)
          </span>
        </div>
        <div 
          onClick={() => setTimeRange(timeRange === 'TUẦN NÀY' ? 'THÁNG NÀY' : 'TUẦN NÀY')}
          className="flex items-center gap-1 text-xs font-display font-semibold text-[#4cd7f6] uppercase tracking-wider cursor-pointer hover:brightness-125 transition-all select-none"
        >
          <span>{timeRange}</span>
      ...
      {/* Wave Chart Box */}
      <div className="relative w-full rounded-xl bg-gradient-to-br from-[#2c2835]/50 to-[#1e1a26]/70 border border-[#373340]/40 p-4 pt-6 overflow-hidden flex flex-col justify-between">
        ...
        <div className="relative w-full h-56 flex items-center">
          ...
          <div className="absolute left-1 top-2 bottom-6 flex flex-col justify-between text-[11px] font-mono font-medium text-on-surface-variant/80 z-10 pointer-events-none">
          ...
          <svg className="w-full h-full pl-8 pb-4 overflow-visible" preserveAspectRatio="none" viewBox="0 0 480 180">
            ...
            <path d={chartData.curvePath} fill="none" stroke="#acedff" strokeLinecap="round" strokeWidth="2.5" />
            
            {/* Interactive Selected Node & Pulse Halo */}
            {selectedPoint && (
              <g>
                <circle cx={selectedPoint.x} cy={selectedPoint.y} fill="#4cd7f6" r="10" opacity="0.3" className="animate-ping" />
                <circle cx={selectedPoint.x} cy={selectedPoint.y} fill="#4cd7f6" r="5" stroke="#ffffff" strokeWidth="2" />
              </g>
            )}
          </svg>
          ...
          {selectedPoint && (
            <div 
              className="absolute z-20 pointer-events-none transform -translate-x-1/2 -translate-y-full bg-[#151025]/95 text-white border border-[#4cd7f6]/40 px-2.5 py-1 rounded-lg shadow-xl text-center"
              style={{
                left: `calc(${(selectedPoint.x / 480) * 100}% + 16px)`,
                top: `${(selectedPoint.y / 180) * 100 - 12}%`
              }}
            >
              <span className="text-[10px] font-mono text-[#4cd7f6] block font-bold leading-tight">{selectedPoint.day}</span>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-[#373340]/40 text-xs font-mono text-on-surface-variant px-2 select-none">
          <button 
            type="button"
            onClick={() => {
            ...
            className="hover:text-[#4cd7f6] transition-colors p-1"
          >
            ...
            return (
              <span
                ...
                className={`transition-colors cursor-pointer py-1 px-1.5 rounded ${
                  isSelected ? 'text-[#4cd7f6] font-bold bg-[#4cd7f6]/10' : 'hover:text-on-surface'
                }`}
              >
                {day}
              </span>
