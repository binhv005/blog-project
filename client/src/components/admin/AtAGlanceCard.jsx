      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display font-bold text-lg text-on-surface">Tổng quan nhanh</h2>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="flex flex-col rounded-xl bg-[#221e2a] border border-[#373340]/40 p-3 hover:border-[#4cd7f6]/40 transition-colors">
            <div className="w-full h-10 mb-2">
              <svg className="w-full h-full overflow-visible" viewBox="0 0 100 36">
                <path d={sparklines.path3} fill="none" stroke="#4cd7f6" strokeWidth="2" strokeLinecap="round" />
                <circle cx="100" cy="10" fill="#4cd7f6" r="3" stroke="#15111d" strokeWidth="1.5" />
              </svg>
            </div>
            <span className="font-mono font-bold text-xl text-on-surface">{stats.totalPosts}</span>
            <span className="font-display text-xs text-on-surface-variant">Tổng bài viết</span>
          </div>

          {/* Mini Metric 2: Published */}
          <div className="flex flex-col rounded-xl bg-[#221e2a] border border-[#373340]/40 p-3 hover:border-emerald-400/40 transition-colors">
            <div className="w-full h-10 mb-2">
              <svg className="w-full h-full overflow-visible" viewBox="0 0 100 36">
                <path d={sparklines.path2} fill="none" stroke="#10b981" strokeWidth="2" strokeLinecap="round" />
                <circle cx="100" cy="12" fill="#10b981" r="3" stroke="#15111d" strokeWidth="1.5" />
              </svg>
            </div>
            <span className="font-mono font-bold text-xl text-emerald-400">{stats.publishedCount}</span>
            <span className="font-display text-xs text-on-surface-variant">Đã xuất bản</span>
          </div>

          {/* Mini Metric 3: Total Views */}
          <div className="flex flex-col rounded-xl bg-[#221e2a] border border-[#373340]/40 p-3 hover:border-[#ec4899]/40 transition-colors">
            <div className="w-full h-10 mb-2">
              <svg className="w-full h-full overflow-visible" viewBox="0 0 100 36">
                <path d={sparklines.path1} fill="none" stroke="#ec4899" strokeWidth="2" strokeLinecap="round" />
                <circle cx={sparklines.lastPt1.x} cy={sparklines.lastPt1.y} fill="#ec4899" r="3" stroke="#15111d" strokeWidth="1.5" />
              </svg>
            </div>
            ...
            <span className="font-display text-xs text-on-surface-variant">Tổng lượt đọc</span>
          </div>
        </div>
      </div>

      {/* System Status & Performance Indicators */}
      <div className="rounded-xl bg-[#221e2a]/80 border border-[#373340]/50 p-4 flex flex-col gap-3">
        <div className="flex items-center justify-between text-xs font-display">
          ...
          <span className={`font-mono font-semibold ${isDatabaseConnected ? 'text-emerald-400' : 'text-amber-400'}`}>
            {isDatabaseConnected ? 'MongoDB Live (dudi_blog)' : 'Bộ nhớ Cache Cục bộ'}
          </span>
        </div>
        <div className="flex items-center justify-between text-xs font-display">
          <span className="text-on-surface-variant">Chuyên mục đang quản lý</span>
          <span className="text-purple-300 font-mono font-semibold">{distinctCategories} danh mục</span>
        </div>
        <div className="flex items-center justify-between text-xs font-display">
          <span className="text-on-surface-variant">Bài viết nháp</span>
          <span className="text-amber-400 font-mono font-semibold">{stats.draftCount} bài nháp</span>
        </div>
      </div>
    </section>
  );
